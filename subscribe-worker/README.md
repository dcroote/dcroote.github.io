...
Cloudflare's production settings live in `wrangler.toml`. After Turnstile
passes, the rate limiter allows five Buttondown attempts per IP per minute.
Turnstile tokens must be issued for `www.derekcroote.com` with the
`newsletter_subscribe` action. Native HTML form posts without JavaScript
receive an HTML error asking the visitor to enable JavaScript.
